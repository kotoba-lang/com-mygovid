(ns mygovid.main
  "Kotodama WASM entrypoint for the mygovid clean-room actor (L4) — Clojure port.
  ...
  facts (`mygovid.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "mygovid")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"mygovid.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "mygovid-compat" :tier tier :entities entities} 200])
...
(defn start! [] :mygovid-compat/ready)
